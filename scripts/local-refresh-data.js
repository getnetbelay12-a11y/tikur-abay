...
const stages = [
  ...
  {
    label: 'Seed local data',
    command: 'pnpm',
    args: ['seed:local'],
  },
  {
    label: 'Seed integrity check',
    command: 'node',
    args: ['scripts/local-seed-integrity-check.js'],
  },
  {
...
runChecks().catch((error) => {
  console.error(`Local data refresh failed: ${error.message}`);
  process.exit(1);
});

async function runChecks() {
  console.log('Running Tikur Abay local data refresh...');
  const results = [];
