...
var AiCommandCenterService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.AiCommandCenterService = void 0;
...
let AiCommandCenterService = AiCommandCenterService_1 = class AiCommandCenterService {
    constructor() {
        this.logger = new common_1.Logger(AiCommandCenterService_1.name);
        this.timers = [];
    }
    async onModuleInit() {
        const queue = (delayMs, task, label, intervalMs) => {
    ...
    async getCommandCenterPayload() {
        ...
        const latestSnapshot = await models_1.AiSnapshotModel.findOne().sort({ generatedAt: -1 }).lean();
        if (!latestSnapshot) {
            await this.refreshAll();
        }
        const snapshot = latestSnapshot || await models_1.AiSnapshotModel.findOne().sort({ generatedAt: -1 }).lean();
    ...
    async generateRecommendedActions() {
        ...
        const [delayedTrips, overdueInvoices, kycQueue, idleFleet] = await Promise.all([
            models_1.AiInsightModel.find({ type: 'risk', category: 'trip', status: 'active' }).sort({ score: -1 }).limit(6).lean(),
            models_1.AiInsightModel.find({ type: 'risk', category: 'finance', status: 'active' }).sort({ score: -1 }).limit(6).lean(),
            models_1.AiInsightModel.find({ type: 'risk', category: 'kyc', status: 'active' }).sort({ score: -1 }).limit(6).lean(),
            models_1.AiInsightModel.find({ type: 'opportunity', category: 'vehicle', status: 'active' }).sort({ score: -1 }).limit(6).lean(),
        ]);
        const actions = [];
    ...
    async generateDashboardSnapshot() {
        ...
        await models_1.AiSnapshotModel.create({
        ...
        const staleSnapshots = await models_1.AiSnapshotModel.find().sort({ generatedAt: -1 }).skip(24).select('_id').lean();
        if (staleSnapshots.length) {
            await models_1.AiSnapshotModel.deleteMany({ _id: { $in: staleSnapshots.map((item) => item._id) } });
        }
    }
    async listInsights(type) {
        const rows = await models_1.AiInsightModel.find({ type, status: 'active' }).sort({ score: -1, updatedAt: -1 }).limit(3).lean();
        return rows.map((row) => ({
            title: row.title,
    ...
    buildSummaryText(risks, opportunities, actions) {
    ...
    buildStats(risks, opportunities, actions) {
        return {
    ...
    async replaceInsights(type, insights) {
        await models_1.AiInsightModel.deleteMany({ type });
        if (!insights.length)
            return;
        await models_1.AiInsightModel.insertMany(insights.map((item) => ({ ...item, status: item.status || 'active' })));
    }
    async computeDelayedTripRisks() {
        const delayedTrips = await models_1.TripModel.find({ status: 'delayed' })
            .sort({ updatedAt: -1 })
            .limit(20)
        ...
        return delayedTrips.map((trip) => {
            const delayedMinutes = Math.max(Number(trip.delayedMinutes ?? 0), trip.plannedArrivalAt ? Math.max(0, Math.round((Date.now() - new Date(trip.plannedArrivalAt).getTime()) / 60000)) : 0);
    ...
    async computeStaleGpsRisks() {
        const vehicles = await models_1.VehicleModel.find({
    ...
    async computeMaintenanceRisks() {
        const plans = await models_1.MaintenancePlanModel.find({
    ...
    async computeInvoiceRisks() {
        const invoices = await models_1.InvoiceModel.find({
    ...
    async computeKycRisks() {
        const rows = await models_1.DriverKycRequestModel.find({ status: { $in: ['submitted', 'under_review', 'draft'] } })
            .sort({ createdAt: 1 })
            .limit(12)
    ...
    async computeIdleFleetOpportunities() {
        const [idleVehicles, demandTrips] = await Promise.all([
            models_1.VehicleModel.find({
                ...
                .lean(),
            models_1.TripModel.find({ status: { $in: ['assigned', 'pending_dispatch'] } })
                .limit(40)
                .select('branchName routeName')
                .lean(),
        ]);
        const demandByBranch = new Map();
    ...
    async computeCollectionOpportunities() {
        const invoices = await models_1.InvoiceModel.find({
            status: { $in: ['pending', 'partially_paid', 'overdue'] },
            outstandingAmount: { $gt: 0 },
        })
            ...
            .lean();
        const payments = await models_1.PaymentModel.find().sort({ paymentDate: -1 }).limit(80).select('customerId customerName amount paymentDate status').lean();
        const customerRecoveryMap = new Map();
        payments.forEach((payment) => {
            const key = String(payment.customerId || payment.customerName || '');
            if (!key || payment.status !== 'paid')
                return;
    ...
    async computeQuoteConversionOpportunities() {
        const [quotes, bookings] = await Promise.all([
            models_1.QuoteModel.find({ status: { $in: ['approved', 'pending', 'requested'] } })
                .sort({ updatedAt: -1 })
                .limit(20)
                .select('_id quoteCode customerName quotedAmount status route requestedVehicleType')
                .lean(),
            models_1.BookingModel.find().select('quoteId').lean(),
        ]);
    ...
    async computeBranchCapacityOpportunities() {
        const [drivers, vehicles, notifications] = await Promise.all([
            models_1.DriverModel.find({ status: 'active' }).select('_id branchId driverCode').lean(),
            models_1.VehicleModel.find({ readyForAssignment: true, currentTripId: null }).select('_id branchId vehicleCode').lean(),
            models_1.NotificationModel.find({ isRead: false }).sort({ createdAt: -1 }).limit(30).select('type entityType entityId').lean(),
        ]);
...
exports.AiCommandCenterService = AiCommandCenterService;
exports.AiCommandCenterService = AiCommandCenterService = AiCommandCenterService_1 = __decorate([
    (0, common_1.Injectable)()
], AiCommandCenterService);
function countByBranch(rows) {
...
//# sourceMappingURL=ai-command-center.service.js.map