...
exports.ExecutiveCommunicationsService = void 0;
const common_1 = require("@nestjs/common");
const mongo_1 = require("../../database/mongo");
const models_1 = require("../../database/models");
let ExecutiveCommunicationsService = class ExecutiveCommunicationsService {
    async getHistory(entityType, entityId, user) {
        await (0, mongo_1.connectToDatabase)();
        const context = await this.resolveEntityContext(entityType, entityId);
        const history = await models_1.ExecutiveCommunicationModel.find({ entityType, entityId })
            .sort({ sentAt: -1, scheduledAt: -1, createdAt: -1 })
            .limit(40)
            .lean();
        return {
            entity: context,
            history: history.map((item) => this.mapRecord(item)),
        };
    }
    async sendCommunication(body, user) {
        await (0, mongo_1.connectToDatabase)();
        const entityType = String(body.entityType || '').trim();
        const entityId = String(body.entityId || '').trim();
        if (!entityType || !entityId) {
            throw new common_1.BadRequestException('Entity reference is required');
        }
        const context = await this.resolveEntityContext(entityType, entityId);
        const channels = expandChannels(body.channel || 'email');
        const template = (body.template || context.defaultTemplate || 'dispatch_action_notice');
        ...
        for (const channel of channels) {
            ...
            if (!recipient) {
                throw new common_1.BadRequestException(`Recipient is required for ${channel}`);
            }
            ...
            const record = await models_1.ExecutiveCommunicationModel.create({
                entityType,
                entityId,
    ...
    async resolveEntityContext(entityType, entityId) {
        switch (entityType) {
            case 'payment':
            case 'invoice':
    ...
    async resolveInvoiceContext(entityType, entityId) {
        ...
        const latestPayment = payment || await models_1.PaymentModel.findOne({ invoiceId: invoice._id }).sort({ paymentDate: -1, createdAt: -1 }).lean();
        ...
        const defaultTemplate = invoice.status === 'paid' || latestPayment?.status === 'paid'
            ? 'thank_you'
    ...
    async resolveTripContext(entityId) {
        const trip = await models_1.TripModel.findById(entityId)
    ...
    async resolveVehicleContext(entityType, entityId) {
        const plan = entityType === 'maintenance_plan'
            ? await models_1.MaintenancePlanModel.findById(entityId).lean()
            : null;
        const vehicle = entityType === 'vehicle'
            ? await models_1.VehicleModel.findById(entityId).select('_id vehicleCode branchName currentStatus').lean()
            : plan?.vehicleId
                ? await models_1.VehicleModel.findById(plan.vehicleId).select('_id vehicleCode branchName currentStatus').lean()
                : null;
        if (!vehicle && !plan)
            throw new common_1.BadRequestException('Vehicle context not found');
    ...
    async resolveDriverKycContext(entityId) {
        const request = await models_1.DriverKycRequestModel.findById(entityId)
            .select('_id fullName phone status partnerCompany partnerVehicleCode')
            .lean();
        if (!request)
            throw new common_1.BadRequestException('Driver KYC request not found');
        return {
    ...
    async resolveIncidentContext(entityId) {
        const incident = await models_1.IncidentReportModel.findById(entityId)
...
exports.ExecutiveCommunicationsService = ExecutiveCommunicationsService;
exports.ExecutiveCommunicationsService = ExecutiveCommunicationsService = __decorate([
    (0, common_1.Injectable)()
], ExecutiveCommunicationsService);
async function resolveCustomerRecipients(customerId, customerCode, customerName) {
    ...
    const [profile, user] = await Promise.all([
        profileQuery.length
            ? models_1.CustomerProfileModel.findOne({ $or: profileQuery }).select('companyName contactPerson phone email').lean()
            : null,
        userQuery.length
            ? models_1.UserModel.findOne({ $or: userQuery }).select('email phone').lean()
            : null,
    ]);
...
function resolveScheduleAt(saveAsDraft, scheduleOption, scheduledAt) {
    if (saveAsDraft)
        return scheduledAt ? new Date(scheduledAt) : null;
    if (scheduleOption === 'custom_time' && scheduledAt)
...
function buildMessage(template, context) {
    switch (template) {
        case 'thank_you':
...
function buildProviderResponse(status, channel, recipient, scheduledAt) {
    if (status === 'scheduled') {
...
//# sourceMappingURL=executive-communications.service.js.map