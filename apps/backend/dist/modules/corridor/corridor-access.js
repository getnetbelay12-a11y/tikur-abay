...
exports.actionAuthorization = exports.stageOwnership = exports.roleScopes = exports.roleAliases = void 0;
exports.normalizeCorridorRole = normalizeCorridorRole;
exports.corridorActorFromRequest = corridorActorFromRequest;
exports.canViewScope = canViewScope;
exports.canViewShipment = canViewShipment;
exports.assertCanViewShipment = assertCanViewShipment;
exports.canPerformShipmentAction = canPerformShipmentAction;
exports.assertCanPerformShipmentAction = assertCanPerformShipmentAction;
exports.filterScopedList = filterScopedList;
exports.filterInternalFields = filterInternalFields;
exports.normalizeShipmentStage = normalizeShipmentStage;
exports.buildStageOwnershipRules = buildStageOwnershipRules;
exports.buildActionRules = buildActionRules;
...
function normalizeCorridorRole(role, mobileRole) {
    const baseRole = String(mobileRole || role || 'customer_user').trim();
    return exports.roleAliases[baseRole] ?? baseRole;
}
function corridorActorFromRequest(user, query) {
    if (user) {
        return {
            userId: user.id,
            name: user.name,
    ...
    const role = normalizeCorridorRole(typeof query?.role === 'string' ? query.role : undefined, typeof query?.mobileRole === 'string' ? query.mobileRole : undefined);
    return {
...
function canViewScope(actor, scope) {
    if (!scope)
        return true;
    if (actor.permissions.includes('*'))
        return true;
    const allowed = exports.roleScopes[actor.role] ?? [];
    return allowed.includes(scope);
}
function canViewShipment(actor, shipment, accessRows = []) {
    if (actor.permissions.includes('*'))
        return true;
    if (['super_admin', 'executive_supervisor'].includes(actor.role))
        return true;
    if (['customer_user', 'customer_agent'].includes(actor.role)) {
        return Boolean(actor.customerCode &&
    ...
    if (['internal_driver', 'external_driver'].includes(actor.role)) {
        return accessRows.some((row) => row.role === actor.role &&
    ...
    const normalizedStage = normalizeShipmentStage(shipment.currentStage || shipment.clearanceWorkflowStatus || shipment.workflowState || shipment.status);
    const allowedStageOwners = exports.stageOwnership[normalizedStage] ?? [];
    return allowedStageOwners.includes(actor.role);
}
function assertCanViewShipment(actor, shipment, accessRows = []) {
...
function canPerformShipmentAction(actor, action, shipmentStage) {
    if (actor.permissions.includes('*'))
        return true;
    const allowedRoles = exports.actionAuthorization[action] ?? [];
    if (!allowedRoles.includes(actor.role))
        return false;
    if (!shipmentStage)
        return true;
    const allowedStageOwners = exports.stageOwnership[shipmentStage] ?? [];
    return allowedStageOwners.includes(actor.role) || ['executive_supervisor', 'super_admin'].includes(actor.role);
}
function assertCanPerformShipmentAction(actor, action, shipmentStage) {
    if (!canPerformShipmentAction(actor, action, shipmentStage)) {
        throw new common_1.ForbiddenException(`Role ${actor.role} cannot perform action ${action}`);
    }
}
function filterScopedList(items, scopeField = 'visibilityScope', actor) {
    if (!actor)
        return items;
    return items.filter((item) => canViewScope(actor, item[scopeField]));
}
function filterInternalFields(payload, actor) {
    if (actor.permissions.includes('*'))
        return payload;
    const clone = { ...payload };
...
function normalizeShipmentStage(value) {
    switch (value) {
        case 'booking_quote':
...
function buildStageOwnershipRules() {
    return Object.entries(exports.stageOwnership).map(([stage, roles]) => ({
        stage: stage,
        ownerRoles: roles,
...
function buildActionRules() {
    return Object.entries(exports.actionAuthorization).map(([action, roles]) => ({
        action: action,
        allowedRoles: roles,
        allowedStages: [],
...
//# sourceMappingURL=corridor-access.js.map