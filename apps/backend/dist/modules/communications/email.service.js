...
let EmailService = class EmailService {
    async send(payload) {
        ...
        const localSimulationDefault = nodeEnv !== 'production' &&
        ...
        const smtpPort = Number(process.env.EMAIL_SMTP_PORT ||
            ...
            587);
        const smtpSecure = String(process.env.EMAIL_SMTP_SECURE ||
        ...
        const smtpPass = process.env.EMAIL_SMTP_PASS ||
            ...
            '';
        const sender = formatSender(process.env.EMAIL_SENDER ||
            process.env.MAIL_FROM ||
            'notifications@tikurabay.local');
        const forcedRecipient = process.env.EMAIL_FORCE_TEST_RECIPIENT ||
        ...
        const preferIpv4 = String(process.env.EMAIL_FORCE_IPV4 || 'true').toLowerCase() !== 'false';
...
function renderBrandedEmail(input) {
    const escapedBody = escapeHtml(input.body)
        .split(/\n\s*\n/)
        .map((paragraph) => `<p style="margin:0 0 16px;color:#213547;font-size:16px;line-height:1.75;">${paragraph.replace(/\n/g, '<br />')}</p>`)
        .join('');
    return `
...
function loadLogoAsset() {
    if (cachedLogo !== undefined)
        return cachedLogo;
    const candidates = [
        (0, node_path_1.resolve)(process.cwd(), 'apps/admin/public/branding/tikur-abay-logo.png'),
        (0, node_path_1.resolve)(process.cwd(), '../admin/public/branding/tikur-abay-logo.png'),
        (0, node_path_1.join)(__dirname, '../../../../admin/public/branding/tikur-abay-logo.png'),
        (0, node_path_1.join)(__dirname, '../../../../../apps/admin/public/branding/tikur-abay-logo.png'),
    ];
    for (const candidate of candidates) {
        if ((0, node_fs_1.existsSync)(candidate)) {
            cachedLogo = (0, node_fs_1.readFileSync)(candidate);
            return cachedLogo;
        }
    }
    cachedLogo = null;
...
function formatSender(value) {
    ...
    if (!trimmed)
        return 'Tikur Abay <notifications@tikurabay.local>';
    if (trimmed.includes('<') && trimmed.includes('>'))
        return trimmed;
    if (trimmed.includes('@'))
        return `Tikur Abay <${trimmed}>`;
    return trimmed;
}
...
async function resolveTransportHost(host) {
    ...
    try {
        const result = await (0, promises_1.lookup)(normalizedHost, { family: 4, all: false });
        return {
