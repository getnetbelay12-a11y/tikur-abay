...
exports.canSendCommunication = canSendCommunication;
exports.assertCanSendCommunication = assertCanSendCommunication;
exports.canViewCommunicationHistory = canViewCommunicationHistory;
exports.assertCanViewCommunicationHistory = assertCanViewCommunicationHistory;
...
function canSendCommunication(user, templateKey, entityType) {
    const role = normalizeRole(user);
    if (!role)
        return false;
    if (supervisorRoles.has(role) || (user?.permissions || []).includes('*'))
        return true;
...
function assertCanSendCommunication(user, templateKey, entityType) {
    if (!canSendCommunication(user, templateKey, entityType)) {
        throw new common_1.ForbiddenException('You are not allowed to send this communication');
    }
}
function canViewCommunicationHistory(user, entityType) {
    const role = normalizeRole(user);
    if (!role)
        return false;
    if (supervisorRoles.has(role) || (user?.permissions || []).includes('*'))
        return true;
    if (financeRoles.has(role) && ['finance', 'invoice', 'payment'].includes(String(entityType || '').toLowerCase()))
        return true;
    if (dispatchRoles.has(role) && ['shipment', 'trip'].includes(String(entityType || '').toLowerCase()))
        return true;
    if (yardRoles.has(role) && ['shipment', 'trip'].includes(String(entityType || '').toLowerCase()))
        return true;
    if (releaseRoles.has(role) && ['shipment'].includes(String(entityType || '').toLowerCase()))
        return true;
    if (supportRoles.has(role) && ['support', 'shipment'].includes(String(entityType || '').toLowerCase()))
        return true;
    if (role === 'hr_officer' && ['driver_kyc_request', 'kyc'].includes(String(entityType || '').toLowerCase()))
        return true;
    return false;
}
function assertCanViewCommunicationHistory(user, entityType) {
    if (!canViewCommunicationHistory(user, entityType)) {
        throw new common_1.ForbiddenException('You are not allowed to view this communication history');
    }
}
//# sourceMappingURL=communication-access.js.map