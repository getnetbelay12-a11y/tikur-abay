...
let LaunchService = class LaunchService {
    async list() {
        ...
        const rows = await models_1.LaunchChecklistItemModel.find().sort({ track: 1, dueDate: 1, updatedAt: -1 }).lean();
        const history = await models_1.ActivityLogModel.find({ entityType: 'launch_checklist_item' }).sort({ createdAt: -1 }).limit(30).lean();
        return { items: rows.map((row) => this.serialize(row)), history: history.map((row) => this.serializeHistory(row)) };
    }
    async update(code, user, body) {
        await (0, mongo_1.connectToDatabase)();
        await this.ensureSeed();
        const update = {};
        if (body.status)
        ...
        const row = (await models_1.LaunchChecklistItemModel.findOneAndUpdate({ code }, { $set: update }, { new: true }).lean());
        if (row) {
    ...
    async ensureSeed() {
        const existing = await models_1.LaunchChecklistItemModel.find({}, { code: 1 }).lean();
        ...
        if (missing.length) {
            await models_1.LaunchChecklistItemModel.insertMany(missing);
        }
    }
    serialize(row) {
        return {
            ...
            checklist: Array.isArray(row?.checklist) ? row.checklist.map((item) => String(item)) : [],
...
exports.LaunchService = LaunchService = __decorate([
