...
function boardStatusForVehicle(vehicle) {
    ...
    if (vehicle.currentStatus === 'assigned')
        return 'assigned';
    if (['loading', 'loaded'].includes(String(vehicle.currentStatus)))
        return 'loading';
    if (['in_transit', 'at_checkpoint', 'at_border', 'delayed'].includes(String(vehicle.currentStatus)))
        return 'on_road';
    if (vehicle.currentStatus === 'offloading')
        return 'offloading';
    if (vehicle.currentStatus === 'in_djibouti')
        return 'in_djibouti';
    if (['blocked', 'accident'].includes(String(vehicle.currentStatus)) || vehicle.readyForAssignment === false && vehicle.safetyStatus === 'blocked')
        return 'blocked';
    if (MAINTENANCE_STATUSES.includes(String(vehicle.currentStatus)))
        return 'under_maintenance';
    return vehicle.currentStatus || 'available';
}
function severityForDue(item) {
    if (item.overdue)
        return 'critical';
    if (item.blockedAssignment)
        return 'high';
    return item.criticalFlag ? 'high' : 'medium';
}
function branchToCity(branchName) {
    const value = String(branchName ?? '').toLowerCase();
    if (!value)
        return null;
    if (value.includes('addis'))
        return 'Addis Ababa';
    if (value.includes('adama'))
        return 'Adama';
    if (value.includes('dire dawa'))
        return 'Dire Dawa';
    if (value.includes('djibouti'))
        return 'Djibouti';
    if (value.includes('kombolcha'))
        return 'Kombolcha';
    if (value.includes('gondar'))
        return 'Gondar';
    if (value.includes('modjo'))
        return 'Modjo';
...
function formatLocationLabel(vehicle, trip) {
    const checkpoint = String(trip?.currentCheckpoint ?? '').toLowerCase().trim();
    if (checkpoint === 'djibouti' || trip?.djiboutiFlag)
        return 'Djibouti';
    if (checkpoint === 'at_border')
        return 'Galafi Border';
    if (checkpoint === 'origin')
        return trip?.origin || branchToCity(vehicle?.branchName) || 'Origin';
    if (checkpoint === 'destination')
        return trip?.destination || branchToCity(vehicle?.branchName) || 'Destination';
    if (checkpoint === 'checkpoint') {
        return cityFromCoordinates(vehicle?.currentLocation?.coordinates?.[1] ?? vehicle?.lastKnownLocation?.latitude, vehicle?.currentLocation?.coordinates?.[0] ?? vehicle?.lastKnownLocation?.longitude) || 'En route';
    }
    if (checkpoint)
        return checkpoint.replace(/_/g, ' ');
...
let OperationsService = class OperationsService {
    async resolveBranchScope(user, branch) {
        const branchName = shouldUseRequestedBranch(user) ? branch : user.branch;
        if (!branchName) {
            return {};
        }
        const branchDoc = await models_1.BranchModel.findOne({ name: branchName }).select('_id name').lean();
        return {
            branchName,
            branchId: branchDoc?._id ? String(branchDoc._id) : user.branchId,
        };
    }
    async getFleetSummary(user, branch) {
        ...
        const [vehicles, activeTrips, maintenanceDueSoon, overduePlans, blockedPlans, fuelLogsToday, rentalTrips, activeFuelTrips] = await Promise.all([
            models_1.VehicleModel.find(branchScope.branchName ? { branchName: branchScope.branchName } : {}).lean(),
            models_1.TripModel.find({ ...(branchScope.branchName ? { branchName: branchScope.branchName } : {}), status: { $in: ACTIVE_TRIP_STATUSES } }).select('_id vehicleId vehicleCode branchName status').lean(),
            models_1.MaintenancePlanModel.countDocuments({
            ...
            models_1.MaintenancePlanModel.find({
                ...(branchScope.branchId ? { branchId: branchScope.branchId } : {}),
                status: { $in: ['active', 'due', 'overdue'] },
                overdue: true,
            }).limit(40).lean(),
            ...
            models_1.FuelLogModel.countDocuments({ ...(branchScope.branchId ? { branchId: branchScope.branchId } : {}), date: { $gte: today } }),
            models_1.RentalPartnerTripModel.find({ ...(branchScope.branchId ? { branchId: branchScope.branchId } : {}), status: { $in: ['assigned', 'in_progress'] } }).lean(),
            models_1.FuelLogModel.find({ ...(branchScope.branchId ? { branchId: branchScope.branchId } : {}), date: { $gte: today } }).select('tripId').lean(),
        ]);
    ...
    async getVehicleStatusBoard(user, branch) {
        await (0, mongo_1.connectToDatabase)();
        const branchScope = await this.resolveBranchScope(user, branch);
        const vehicles = await models_1.VehicleModel.find(branchScope.branchName ? { branchName: branchScope.branchName } : {}).sort({ updatedAt: -1 }).limit(200).lean();
        ...
        const [drivers, driverProfiles, trips, overduePlans, activeIssues, fuelLogs, serviceHistory, partReplacements, repairOrders] = await Promise.all([
            models_1.DriverModel.find({ _id: { $in: driverIds } }).select('_id firstName lastName driverCode userId').lean(),
            models_1.DriverProfileModel.find({ driverId: { $in: driverIds } }).select('driverId phone').lean(),
            models_1.TripModel.find({ _id: { $in: tripIds } }).select('_id tripCode status origin destination currentCheckpoint djiboutiFlag branchName').lean(),
            models_1.MaintenancePlanModel.find({ vehicleId: { $in: vehicles.map((item) => item._id) }, status: { $in: ['active', 'due', 'overdue'] } })
                .select('vehicleId serviceItemName overdue blockedAssignment nextDueDate nextDueKm criticalFlag').lean(),
            models_1.DriverReportModel.find({ vehicleId: { $in: vehicles.map((item) => item._id) }, status: { $in: ['submitted', 'under_review'] } })
                .select('vehicleId').lean(),
            models_1.FuelLogModel.find({ vehicleId: { $in: vehicles.map((item) => item._id) } }).sort({ date: -1 }).select('vehicleId date').lean(),
            models_1.VehicleServiceHistoryModel.find({ vehicleId: { $in: vehicles.map((item) => item._id) } }).sort({ serviceDate: -1 }).select('vehicleId serviceCategory serviceDate nextServiceDueDate nextServiceDueKm').lean(),
            models_1.PartReplacementModel.find({ vehicleId: { $in: vehicles.map((item) => item._id) } }).sort({ replacementDate: -1 }).select('vehicleId partName replacementDate').lean(),
            models_1.RepairOrderModel.find({ vehicleId: { $in: vehicles.map((item) => item._id) }, status: { $in: ['reported', 'under_review', 'approved', 'scheduled', 'in_service'] } })
        ...
        const driverUsers = await models_1.UserModel.find({ _id: { $in: drivers.map((item) => item.userId).filter(Boolean) } }).select('_id phone').lean();
        ...
        for (const plan of overduePlans) {
            ...
            if (!planByVehicle.has(key))
                planByVehicle.set(key, []);
            planByVehicle.get(key).push(plan);
        }
        const latestFuelByVehicle = new Map();
        for (const item of fuelLogs) {
            const key = String(item.vehicleId);
            if (!latestFuelByVehicle.has(key))
                latestFuelByVehicle.set(key, item);
        }
        const latestServiceByVehicle = new Map();
        const latestTireByVehicle = new Map();
        for (const item of serviceHistory) {
            const key = String(item.vehicleId);
            if (String(item.serviceCategory).toLowerCase() === 'tire') {
                if (!latestTireByVehicle.has(key))
                    latestTireByVehicle.set(key, item);
            }
            else if (!latestServiceByVehicle.has(key)) {
                latestServiceByVehicle.set(key, item);
            }
        }
        const latestPartByVehicle = new Map();
        for (const item of partReplacements) {
            const key = String(item.vehicleId);
            if (!latestPartByVehicle.has(key))
                latestPartByVehicle.set(key, item);
        }
        const repairOrderByVehicle = new Map();
        for (const order of repairOrders) {
            const key = String(order.vehicleId);
            if (!repairOrderByVehicle.has(key))
                repairOrderByVehicle.set(key, order);
        }
    ...
    async getActiveTrips(user, branch) {
        await (0, mongo_1.connectToDatabase)();
        const branchScope = await this.resolveBranchScope(user, branch);
        const trips = await models_1.TripModel.find({
        ...
        const [drivers, profiles] = await Promise.all([
            models_1.DriverModel.find({ _id: { $in: driverIds } }).select('_id userId firstName lastName').lean(),
            models_1.DriverProfileModel.find({ driverId: { $in: driverIds } }).select('driverId phone').lean(),
        ]);
        const users = await models_1.UserModel.find({ _id: { $in: drivers.map((item) => item.userId).filter(Boolean) } }).select('_id phone phoneNumber').lean();
        const driverById = new Map(drivers.map((item) => [String(item._id), item]));
        ...
        return trips.map((trip) => {
            ...
            return {
                ...
                currentLocation: formatLocationLabel({ branchName: trip.branchName }, {
    ...
    async getAvailableVehicles(user, branch) {
    ...
    async getUnavailableVehicles(user, branch) {
        const board = await this.getVehicleStatusBoard(user, branch);
        const repairOrders = await models_1.RepairOrderModel.find({
    ...
    async getFuelLogs(user, branch) {
        await (0, mongo_1.connectToDatabase)();
        const branchScope = await this.resolveBranchScope(user, branch);
        const today = startOfDay();
        const week = startOfWeek();
        const month = startOfMonth();
        const activeVehicles = await models_1.VehicleModel.countDocuments({ ...(branchScope.branchName ? { branchName: branchScope.branchName } : {}), currentStatus: { $in: ACTIVE_TRIP_STATUSES } });
        const branchFilter = branchScope.branchId ? { branchId: branchScope.branchId } : {};
        const logs = await models_1.FuelLogModel.find(branchFilter)
            .sort({ date: -1 }).limit(40).lean();
        const [driverProfiles, drivers] = await Promise.all([
            models_1.DriverProfileModel.find({ driverId: { $in: logs.map((item) => item.driverId).filter(Boolean) } }).select('driverId phone').lean(),
            models_1.DriverModel.find({ _id: { $in: logs.map((item) => item.driverId).filter(Boolean) } }).select('_id userId').lean(),
        ]);
        const users = await models_1.UserModel.find({ _id: { $in: drivers.map((item) => item.userId).filter(Boolean) } }).select('_id phone').lean();
        const driverProfileByDriverId = new Map(driverProfiles.map((item) => [String(item.driverId), item]));
        const driverById = new Map(drivers.map((item) => [String(item._id), item]));
        ...
        const [todayLogs, weekLiters, monthCost] = await Promise.all([
            models_1.FuelLogModel.countDocuments({ date: { $gte: today }, ...branchFilter }),
            models_1.FuelLogModel.aggregate([{ $match: { date: { $gte: week }, ...branchFilter } }, { $group: { _id: null, total: { $sum: '$liters' } } }]),
            models_1.FuelLogModel.aggregate([{ $match: { date: { $gte: month }, ...branchFilter } }, { $group: { _id: null, total: { $sum: '$cost' } } }]),
        ]);
    ...
    async getMaintenanceDue(user, branch) {
        await (0, mongo_1.connectToDatabase)();
        const branchScope = await this.resolveBranchScope(user, branch);
        const rows = await models_1.MaintenancePlanModel.find({
            ...(branchScope.branchId ? { branchId: branchScope.branchId } : {}),
            status: { $in: ['active', 'due', 'overdue'] },
        ...
        const repairOrderByVehicleId = new Map();
        for (const order of repairOrders) {
            const key = String(order.vehicleId);
    ...
    async getPartsHistory(user, branch) {
        await (0, mongo_1.connectToDatabase)();
        const branchScope = await this.resolveBranchScope(user, branch);
        const vehicles = await models_1.VehicleModel.find(branchScope.branchName ? { branchName: branchScope.branchName } : {}).sort({ updatedAt: -1 }).limit(120).lean();
        const vehicleIds = vehicles.map((item) => item._id);
        const [serviceHistory, partReplacements] = await Promise.all([
            models_1.VehicleServiceHistoryModel.find({ vehicleId: { $in: vehicleIds } }).sort({ serviceDate: -1 }).lean(),
            models_1.PartReplacementModel.find({ vehicleId: { $in: vehicleIds } }).sort({ replacementDate: -1 }).lean(),
        ]);
    ...
    async getRentalPartnerPerformance(user, branch) {
        await (0, mongo_1.connectToDatabase)();
        const branchScope = await this.resolveBranchScope(user, branch);
        const [partners, trips] = await Promise.all([
            models_1.RentalPartnerModel.find({ status: 'active' }).sort({ partnerName: 1 }).lean(),
            models_1.RentalPartnerTripModel.find({ ...(branchScope.branchId ? { branchId: branchScope.branchId } : {}) }).lean(),
        ]);
    ...
    async getRecommendedRentalPartners(user, branch) {
    ...
    async createVehicleServiceHistory(user, body) {
        await (0, mongo_1.connectToDatabase)();
        const doc = await models_1.VehicleServiceHistoryModel.create({
            ...body,
        ...
        if (doc.vehicleId) {
            ...
            await models_1.VehicleModel.updateOne({ _id: doc.vehicleId }, { $set: update });
        }
        return doc.toObject();
    }
    async createPartReplacement(user, body) {
        await (0, mongo_1.connectToDatabase)();
        const doc = await models_1.PartReplacementModel.create({
            ...body,
        ...
        if (doc.vehicleId && String(doc.partCategory).toLowerCase() === 'tire') {
            await models_1.VehicleModel.updateOne({ _id: doc.vehicleId }, { $set: { lastTireChangeAt: doc.replacementDate, lastTireChangeKm: doc.replacementKm } });
        }
        return doc.toObject();
    }
};
