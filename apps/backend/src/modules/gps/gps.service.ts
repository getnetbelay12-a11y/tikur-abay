...
function markerColor(status: string, offline: boolean) {
  if (offline) return '#6b7280';
  if (status === 'delayed') return '#dc2626';
  if (status === 'in_djibouti') return '#0f766e';
  if (status === 'loading' || status === 'loaded') return '#d97706';
  if (status === 'under_maintenance' || status === 'breakdown') return '#7c3aed';
  return '#2563eb';
}
...
export class GpsService {
  ...
  async getLiveFleet(filters: LiveFleetQueryDto = {}) {
    ...
    if (filters.branch) vehicleQuery.branchName = filters.branch;
    if (filters.vehicleStatus) vehicleQuery.currentStatus = filters.vehicleStatus;
    ...
    const tripQuery: Record<string, unknown> = { _id: { $in: tripIds } };
    if (filters.route) tripQuery.routeName = filters.route;
    if (filters.tripStatus) tripQuery.status = filters.tripStatus;
    if (filters.geofence === 'djibouti') tripQuery.djiboutiFlag = true;
    ...
    const [trips, drivers, driverProfiles, latestGpsPoints, latestFuelLogs] = await Promise.all([
      TripModel.find(tripQuery)
        ...
        .lean(),
      DriverModel.find({ _id: { $in: driverIds } }).select('_id firstName lastName').lean(),
      DriverProfileModel.find({ driverId: { $in: driverIds } }).select('driverId phone').lean(),
      GpsPointModel.find({ vehicleId: { $in: vehicles.map((vehicle) => vehicle._id) } })
        .sort({ recordedAt: -1 })
        .select('vehicleId speed recordedAt latitude longitude geofence')
        .lean(),
      FuelLogModel.find({ vehicleId: { $in: vehicles.map((vehicle) => vehicle._id) } })
        .sort({ date: -1 })
    ...
    const latestGpsByVehicleId = newestByKey(latestGpsPoints as any[], 'vehicleId');
    const latestFuelByVehicleId = newestByKey(latestFuelLogs as any[], 'vehicleId');
    ...
    const fleet = vehicles.map((vehicle) => {
      ...
      if (filters.vehicleStatus && item.vehicleStatus !== filters.vehicleStatus) return false;
      if (filters.geofence && item.geofence !== filters.geofence) return false;
      if (filters.djiboutiOnly === 'true' && !item.djiboutiFlag) return false;
      if (filters.delayedOnly === 'true' && !item.delayed) return false;
      if (filters.offlineOnly === 'true' && !item.offline) return false;
      return true;
    });
  ...
  async getVehicleLive(vehicleId: string) {
    await connectToDatabase();
    ...
    const [latestPoint, activeTrip, driver, driverProfile, latestFuel] = await Promise.all([
      GpsPointModel.findOne({ vehicleId: vehicle._id }).sort({ recordedAt: -1 }).lean(),
      vehicle.currentTripId ? TripModel.findById(vehicle.currentTripId).lean() : null,
      vehicle.assignedDriverId ? DriverModel.findById(vehicle.assignedDriverId).lean() : null,
      vehicle.assignedDriverId ? DriverProfileModel.findOne({ driverId: vehicle.assignedDriverId }).select('phone emergencyContact').lean() : null,
      FuelLogModel.findOne({ vehicleId: vehicle._id }).sort({ date: -1 }).select('date station').lean(),
    ]);
  ...
  async getTripHistory(tripId: string) {
    await connectToDatabase();
    const trip = await TripModel.findOne({ $or: [{ _id: tripId }, { tripCode: tripId }] }).lean();
    if (!trip) {
      return [];
    }
    const points = await GpsPointModel.find({ tripId: trip._id }).sort({ recordedAt: 1 }).lean();
  ...
  async recordGpsPoint(
    ...
    const trip = body.tripId
      ? await TripModel.findOne({ $or: [{ _id: body.tripId }, { tripCode: body.tripId }] }).lean()
      : vehicle.currentTripId
        ? await TripModel.findById(vehicle.currentTripId).lean()
        : null;

    const point = await GpsPointModel.create({
  ...
  async getMapWidgetData(filters: LiveFleetQueryDto = {}) {
...
function formatLocationLabel({
  ...
  if (city && geofence) return `${city} · ${humanizeGeofence(geofence)}`;
  if (city) return city;
  if (geofence) return `${humanizeGeofence(geofence)} · ${routeName || branch || 'Active route'}`;
  return branch || routeName || 'Location update pending';
}

function humanizeGeofence(value?: string | null) {
  const normalized = String(value || '').trim();
  if (!normalized) return 'On corridor';
