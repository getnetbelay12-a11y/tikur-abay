import { BookingModel, CorridorShipmentModel } from '../../database/models';

function buildCorridorRoute(booking: any) {
  const origin =
    String(booking.routeSnapshot?.portOfLoading || booking.routeSnapshot?.originCityOrPort || booking.routeSnapshot?.placeOfReceipt || '').trim();
  const destination =
    String(booking.routeSnapshot?.inlandDestination || booking.routeSnapshot?.finalDeliveryLocation || booking.routeSnapshot?.portOfDischarge || '').trim();
  if (origin && destination) return `${origin} -> ${destination}`;
  return String(booking.route || booking.routeSnapshot?.routeSummary || 'China -> Djibouti -> Ethiopia');
}

function buildShipmentPatchFromBooking(booking: any) {
...
export async function materializeCorridorShipmentFromBooking(reference: string) {
  ...
  const shipmentRef = String(booking.shipmentRef || booking.bookingCode || '').trim();
  if (!shipmentRef) return null;

  await CorridorShipmentModel.findOneAndUpdate(
    { shipmentRef },
    { $set: buildShipmentPatchFromBooking(booking) },
    { upsert: true, new: true },
  ).lean();

  return CorridorShipmentModel.findOne({ shipmentRef }).lean();
}
