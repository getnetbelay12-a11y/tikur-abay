...
type BranchSeed = { code: string; name: string; city: string; country: string };
...
function pickWeighted<T>(rng: () => number, items: Array<{ value: T; weight: number }>) {
...
function seedPhoneNumber(index: number) {
...
function seedTemplate(templateKey: string, entityType: string, channel: string, language: string, subjectTemplate: string, bodyTemplate: string, variables: string[]) {
  return {
...
function generatedUserRole(index: number) {
  if (index <= 31) return 'driver';
  if (index <= 32) return 'dispatcher';
  if (index <= 33) return 'operations_manager';
  if (index <= 34) return 'technical_manager';
  if (index <= 35) return 'finance_officer';
  if (index <= 36) return 'hr_officer';
  if (index <= 37) return 'marketing_officer';
  return 'customer';
}

function dateOffset(days: number, hours = 0) {
...
function safeDateFrom(value: unknown, fallback = new Date()) {
  const parsed = new Date(String(value ?? ''));
  return Number.isNaN(parsed.getTime()) ? new Date(fallback) : parsed;
}

function toGeoPoint(point: { latitude: number; longitude: number }) {
  return {
    type: 'Point',
...
function toLegacyPoint(point: { latitude: number; longitude: number }) {
  return {
...
function seededPersonName(index: number) {
...
function seededCustomerCompany(index: number) {
...
function tripCheckpointForStatus(status: string, route: (typeof routeCatalog)[number]) {
  if (status === 'assigned') return route.origin;
  if (status === 'loading' || status === 'loaded') return route.origin;
  if (status === 'at_border') return 'at_border';
  if (status === 'in_djibouti') return 'djibouti';
  if (status === 'offloading') return route.destination;
  if (status === 'delayed') return route.points[Math.min(1, route.points.length - 1)]?.geofence ?? 'checkpoint';
  return route.points[Math.min(1, route.points.length - 1)]?.geofence ?? 'checkpoint';
}

function routePointForStatus(status: string, route: (typeof routeCatalog)[number]) {
  if (status === 'assigned' || status === 'loading' || status === 'loaded') return route.points[0]!;
  if (status === 'at_border') return route.points[Math.min(route.points.length - 1, Math.max(route.points.length - 2, 0))]!;
  if (status === 'in_djibouti' || status === 'offloading') return route.points[route.points.length - 1]!;
...
async function seedBaseData() {
  ...
  await RoleModel.bulkWrite(roleWrites);
  ...
  await BranchModel.bulkWrite(branchWrites);

  const branchDocs = await BranchModel.find({ code: { $in: branches.map((item) => item.code) } }).lean();
  ...
  await UserModel.bulkWrite(userWrites);
  ...
  await EmployeeModel.bulkWrite(employeeWrites);
  ...
  await DriverModel.bulkWrite(driverWrites);
}
...
export async function seedLocalData() {
  ...
  const users = await UserModel.find().lean();
  const branchDocs = await BranchModel.find().lean();
  const branchByCode = new Map(branchDocs.map((branch) => [branch.code, branch]));
  const customers = [];
  ...
  await CustomerModel.bulkWrite(customerWrites);
  customers.push(...(await CustomerModel.find().lean()));
  ...
  await CustomerProfileModel.bulkWrite(customerProfileWrites);
  ...
  await DriverProfileModel.bulkWrite(driverProfileWrites);
  ...
  await DriverKycRequestModel.bulkWrite(driverKycWrites);
  ...
  await VehicleModel.bulkWrite(vehicleWrites);
  ...
  const tripWrites = [];
  const delayedTripIndexes = new Set<number>();
  const now = new Date();
  ...
  await TripModel.bulkWrite(tripWrites);
  ...
  const vehicleStatusWrites = vehicles.map((vehicle, index) => {
    ...
    const currentStatus =
      hqVehicleIds.has(String(vehicle._id))
  ...
  guaranteedMaintenanceVehicles.forEach((vehicle, index) => {
    const branchRoutePoint =
      routeCatalog.find((route) => route.origin === vehicle.branchName || route.destination === vehicle.branchName)?.points[0]
  ...
  await VehicleModel.bulkWrite(vehicleStatusWrites);
  ...
  await TripEventModel.bulkWrite(tripEventWrites);
  ...
  for (const trip of trips) {
    ...
    for (let pointIndex = 0; pointIndex < seedVolumes.tripGpsPointsPerTrip; pointIndex += 1) {
      const anchor = route.points[
        Math.min(route.points.length - 1, Math.floor((pointIndex / seedVolumes.tripGpsPointsPerTrip) * route.points.length))
      ]!;
  ...
  for (let index = 0; index < gpsWrites.length; index += 2000) {
    await GpsPointModel.bulkWrite(gpsWrites.slice(index, index + 2000));
  }
  ...
  await MaintenancePlanModel.bulkWrite(maintenancePlanWrites);
  ...
  await MaintenanceRecordModel.bulkWrite(maintenanceRecordWrites);
  ...
  await RepairOrderModel.bulkWrite(repairOrderWrites);
  ...
  await SparePartModel.bulkWrite(sparePartWrites);
  ...
  await MaintenanceNotificationModel.bulkWrite(maintenanceNotificationWrites);
  ...
  await DriverReportModel.bulkWrite(driverReportWrites);
  ...
  await AgreementModel.bulkWrite(agreementWrites);
  const agreements = await AgreementModel.find().lean();
  ...
  await AgreementSignatureModel.bulkWrite(agreementSignatureWrites);
  ...
  await InvoiceModel.bulkWrite(invoiceWrites);
  ...
  await PaymentModel.bulkWrite(paymentWrites);
  ...
  for (let index = 1; index <= seedVolumes.paymentCommunications; index += 1) {
    ...
    const invoice = invoices.find((entry: any) => String(entry._id) === String(payment.invoiceId)) || invoices[index % invoices.length]!;
  ...
  await PaymentCommunicationModel.bulkWrite(paymentCommunicationWrites);

  const executiveCommunicationWrites = [];
  const maintenancePlans = await MaintenancePlanModel.find().lean();
  const tripRows = await TripModel.find().lean();
  const kycRows = await DriverKycRequestModel.find().lean();
  const incidentRows = await IncidentReportModel.find().lean();
  const communicationModes = [0, 1, 2];
  if (kycRows.length) communicationModes.push(3);
  if (incidentRows.length) communicationModes.push(4);
  ...
  await ExecutiveCommunicationModel.bulkWrite(executiveCommunicationWrites);
  ...
  await CommunicationTemplateModel.bulkWrite(communicationTemplateWrites);
  ...
  await CommunicationAutomationRuleModel.bulkWrite(automationRuleWrites);
  ...
  for (let index = 1; index <= seedVolumes.communicationLogs; index += 1) {
    ...
    const templateKey =
      mode === 0 ? (index % 3 === 0 ? 'payment_receipt' : index % 2 === 0 ? 'payment_thank_you' : 'payment_reminder')
      : mode === 1 ? 'trip_delay_update'
      : mode === 2 ? 'maintenance_escalation'
      : mode === 3 ? 'kyc_reminder'
      : 'incident_acknowledgement';
    const entity =
      mode === 0 ? invoices[index % invoices.length]
  ...
  await CommunicationLogModel.bulkWrite(communicationLogWrites);
  const corridorShipments = await CorridorShipmentModel.find().lean();
  const corridorTrips = await CorridorTripAssignmentModel.find().lean();
  ...
  await CommunicationDraftModel.bulkWrite(communicationDraftWrites);
  ...
  await CommunicationScheduleModel.bulkWrite(communicationScheduleWrites);
  ...
  await NotificationEventModel.bulkWrite(notificationEventWrites);
  ...
  await QuoteModel.bulkWrite(quoteWrites);
  ...
  await AvailabilityReportModel.bulkWrite(availabilityWrites);
  ...
  await LeaveRequestModel.bulkWrite(leaveWrites);
  ...
  await EmployeePerformanceMetricModel.bulkWrite(employeePerfWrites);
  ...
  await DriverPerformanceMetricModel.bulkWrite(driverPerfWrites);

  const notificationWrites = [];
  const notificationUsers = await UserModel.find().limit(seedVolumes.users).lean();
  ...
  for (let index = 0; index < notificationWrites.length; index += 1000) {
    await NotificationModel.bulkWrite(notificationWrites.slice(index, index + 1000));
  }
  ...
  for (let index = 0; index < documentWrites.length; index += 1000) {
    await DocumentModel.bulkWrite(documentWrites.slice(index, index + 1000));
  }
  ...
  await UserPreferenceModel.bulkWrite(preferenceWrites);
  ...
  for (let index = 0; index < fuelWrites.length; index += 400) {
    await FuelLogModel.bulkWrite(fuelWrites.slice(index, index + 400));
  }
  ...
  for (let index = 0; index < serviceHistoryWrites.length; index += 400) {
    await VehicleServiceHistoryModel.bulkWrite(serviceHistoryWrites.slice(index, index + 400));
  }
  ...
  for (let index = 0; index < partReplacementWrites.length; index += 400) {
    await PartReplacementModel.bulkWrite(partReplacementWrites.slice(index, index + 400));
  }
  ...
  await RentalPartnerModel.bulkWrite(rentalPartnerWrites);

  const rentalPartners = await RentalPartnerModel.find().lean();
  ...
  for (let index = 0; index < rentalPartnerTripWrites.length; index += 300) {
    await RentalPartnerTripModel.bulkWrite(rentalPartnerTripWrites.slice(index, index + 300));
  }
  ...
  for (let index = 0; index < incidentWrites.length; index += 300) {
    await IncidentReportModel.bulkWrite(incidentWrites.slice(index, index + 300));
  }
  ...
  for (let index = 0; index < activityWrites.length; index += 500) {
    await ActivityLogModel.bulkWrite(activityWrites.slice(index, index + 500));
  }
  ...
  for (let index = 0; index < uploadedDocumentWrites.length; index += 400) {
    await UploadedDocumentModel.bulkWrite(uploadedDocumentWrites.slice(index, index + 400));
  }
  ...
  await CollectionTaskModel.bulkWrite(collectionTaskWrites);

  const collectionTasks = await CollectionTaskModel.find().lean();
  ...
  await EscalationLogModel.bulkWrite(escalationWrites);
  ...
  await LeadModel.bulkWrite(leadWrites);
  ...
  await OnboardingTaskModel.bulkWrite(onboardingWrites);
  ...
  await OutboundNotificationModel.bulkWrite(outboundWrites);
  ...
  await JobRequisitionModel.bulkWrite(requisitionWrites);

  const requisitions = await JobRequisitionModel.find().lean();
  ...
  await CandidateModel.bulkWrite(candidateWrites);
  ...
  await OnboardingTaskHrModel.bulkWrite(onboardingHrWrites);
  ...
  await TrainingRecordModel.bulkWrite(trainingWrites);
  ...
  await DrivingSchoolStudentModel.bulkWrite(drivingSchoolStudentWrites);

  const drivingSchoolStudents = await DrivingSchoolStudentModel.find().lean();
  ...
  await DrivingSchoolPaymentModel.bulkWrite(drivingSchoolPaymentWrites);
...
async function seedCorridorData() {
  const [customers, customerProfiles, drivers, driverProfiles, vehicles, invoices, payments, supportRooms] = await Promise.all([
    CustomerModel.find().sort({ createdAt: 1 }).limit(12).lean(),
    CustomerProfileModel.find().sort({ createdAt: 1 }).limit(12).lean(),
    DriverModel.find().sort({ createdAt: 1 }).limit(16).lean(),
    DriverProfileModel.find().sort({ createdAt: 1 }).limit(16).lean(),
    VehicleModel.find().sort({ createdAt: 1 }).limit(16).lean(),
    InvoiceModel.find().sort({ createdAt: -1 }).limit(24).lean(),
    PaymentModel.find().sort({ createdAt: -1 }).limit(24).lean(),
    ChatRoomModel.find().sort({ createdAt: -1 }).limit(12).lean(),
  ]);
  ...
  for (let index = 0; index < 10; index += 1) {
    ...
    const customerCity = String((profile as any)?.city || (customer as any)?.city || dryPortNode);
  ...
  await CorridorShipmentModel.bulkWrite(shipmentWrites);
  await CorridorPartyAccessModel.bulkWrite(accessWrites);
  await CorridorCargoItemModel.bulkWrite(cargoItemWrites);
  await CorridorDocumentModel.bulkWrite(documentWrites);
  await CorridorContainerModel.bulkWrite(containerWrites);
  if (tripWrites.length) await CorridorTripAssignmentModel.bulkWrite(tripWrites);
  if (milestoneWrites.length) await CorridorMilestoneModel.bulkWrite(milestoneWrites);
  if (exceptionWrites.length) await CorridorExceptionModel.bulkWrite(exceptionWrites);
  if (emptyReturnWrites.length) await CorridorEmptyReturnModel.bulkWrite(emptyReturnWrites);
  if (checkpointWrites.length) await CorridorCheckpointEventModel.bulkWrite(checkpointWrites);
...
export async function resetLocalData() {
  ...
  await Promise.all([
    ...
    TripEventModel.deleteMany({}),
    GpsPointModel.deleteMany({}),
    MaintenancePlanModel.deleteMany({}),
    MaintenanceRecordModel.deleteMany({}),
    MaintenanceNotificationModel.deleteMany({}),
    DriverReportModel.deleteMany({}),
    AgreementModel.deleteMany({}),
    InvoiceModel.deleteMany({}),
    PaymentModel.deleteMany({}),
    PaymentCommunicationModel.deleteMany({}),
    ExecutiveCommunicationModel.deleteMany({}),
    CommunicationDraftModel.deleteMany({}),
    CommunicationLogModel.deleteMany({}),
    CommunicationScheduleModel.deleteMany({}),
    CommunicationTemplateModel.deleteMany({}),
    CommunicationAutomationRuleModel.deleteMany({}),
    NotificationEventModel.deleteMany({}),
    CorridorShipmentModel.deleteMany({}),
    CorridorPartyAccessModel.deleteMany({}),
    CorridorCargoItemModel.deleteMany({}),
    CorridorDocumentModel.deleteMany({}),
    CorridorContainerModel.deleteMany({}),
    CorridorTripAssignmentModel.deleteMany({}),
    CorridorMilestoneModel.deleteMany({}),
    CorridorExceptionModel.deleteMany({}),
    CorridorCheckpointEventModel.deleteMany({}),
    CorridorEmptyReturnModel.deleteMany({}),
    RepairOrderModel.deleteMany({}),
    SparePartModel.deleteMany({}),
    NotificationModel.deleteMany({}),
    DocumentModel.deleteMany({}),
    UserPreferenceModel.deleteMany({}),
    FuelLogModel.deleteMany({}),
    VehicleServiceHistoryModel.deleteMany({}),
    PartReplacementModel.deleteMany({}),
    RentalPartnerModel.deleteMany({}),
    RentalPartnerTripModel.deleteMany({}),
    IncidentReportModel.deleteMany({}),
    ActivityLogModel.deleteMany({}),
    UploadedDocumentModel.deleteMany({}),
    CustomerProfileModel.deleteMany({}),
    DriverProfileModel.deleteMany({}),
    DriverKycRequestModel.deleteMany({}),
    AgreementSignatureModel.deleteMany({}),
    BookingModel.deleteMany({}),
    QuoteModel.deleteMany({}),
    AvailabilityReportModel.deleteMany({}),
    LeaveRequestModel.deleteMany({}),
    CollectionTaskModel.deleteMany({}),
    EscalationLogModel.deleteMany({}),
    LeadModel.deleteMany({}),
    OnboardingTaskModel.deleteMany({}),
    OutboundNotificationModel.deleteMany({}),
    JobRequisitionModel.deleteMany({}),
    CandidateModel.deleteMany({}),
    OnboardingTaskHrModel.deleteMany({}),
    TrainingRecordModel.deleteMany({}),
    DrivingSchoolStudentModel.deleteMany({}),
    DrivingSchoolPaymentModel.deleteMany({}),
    EmployeePerformanceMetricModel.deleteMany({}),
    DriverPerformanceMetricModel.deleteMany({}),
  ]);
}
