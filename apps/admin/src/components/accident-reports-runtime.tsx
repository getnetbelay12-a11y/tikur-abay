...
export function AccidentReportsRuntime({ incidents }: { incidents: IncidentRow[] }) {
  const { tx } = useConsoleI18n();
  const rows = useMemo(
    () => incidents.filter((item) => formatText(item.type, '') === 'accident_report'),
    [incidents],
  );
    ...
    <main className="shell">
      <div className="panel workspace-shell">
        <section className="workspace-header">
          <div className="workspace-header-copy">
            <div className="eyebrow">{tx('Safety Escalation')}</div>
            <h1>{tx('Accident Reports')}</h1>
            <p>{tx('Accident incidents with trip, driver, vehicle, severity, and location context.')}</p>
          </div>
        </section>

        <section className="workspace-kpi-grid">
          <KpiCard label={tx('Total Reports')} value={total} helper={tx('Accident incidents in the register')} />
          <KpiCard label={tx('Open')} value={open} helper={tx('Reported and awaiting first review')} tone={open ? 'warning' : 'good'} />
          <KpiCard label={tx('Under Review')} value={review} helper={tx('Safety and operations follow-up in progress')} tone={review ? 'info' : 'good'} />
          <KpiCard label={tx('Critical')} value={critical} helper={tx('Highest severity accident cases')} tone={critical ? 'critical' : 'good'} />
        </section>

        <WorkspaceFilterBar
          fields={[
            { key: 'search', label: 'Search', type: 'search', value: search, placeholder: 'Trip, vehicle, driver, location', onChange: setSearch },
        ...
        <section className="card workspace-table-card">
          <div className="workspace-section-header">
            <div>
              <div className="eyebrow">{tx('Accident Queue')}</div>
              <h3>{filtered.length} {tx('reports')}</h3>
            </div>
          </div>
          {!filtered.length ? (
            <div className="empty-state inline-state-card"><p>{tx('No accident reports match the current filters.')}</p></div>
          ) : (
      ...
      {selected ? (
        <WorkspaceDetailDrawer
          title={formatText(selected.tripCode, 'Accident report')}
        ...
        >
          ...
          <section className="card workspace-detail-card">
            <div className="workspace-section-header">
              <div>
                <div className="eyebrow">{tx('Incident Detail')}</div>
                <h3>{tx('Accident summary')}</h3>
              </div>
            </div>
            <div className="workspace-detail-list">
              <div className="workspace-detail-row">
                <div className="workspace-cell-stack">
                  ...
                  <span>{formatText(selected.description, tx('Accident report submitted for escalation.'))}</span>
                </div>
...
function locationLabel(value: unknown) {
  ...
  return 'Field location recorded';
}
